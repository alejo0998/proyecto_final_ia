...
def send_video():
    ...
    try:
        ...
        if web == 'true': 
            result = db.from_sequence([nombre_archivo], partition_size=2).map(run_in_subprocess, frames_extraction_web)
        else:
            result = db.from_sequence([nombre_archivo], partition_size=2).map(run_in_subprocess, frames_extraction)
